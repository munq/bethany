		...
		<div id="nav-menu">
			<ul class="nav-main clearfix">
				...
				<?php
					...
					<li>
						<a href="<?php echo $main_menu->url; ?>" <?php echo (!empty($main_menu->classes) ? ' class="'.implode(' ', $main_menu->classes).'"': ''); ?> ><?php echo $main_menu->title; ?></a>
						<?php if (!empty($sub_menus[$db_id]) || $main_menu->classes[0] != '') { ?>
							<div class="nav-mega-wrapper">
							<?php
								if($main_menu->classes[0] != '') {
								...
								<?php foreach($sub_menus[$db_id] as $sub_db_id => $sub_menu) { ?>
									<li>
										<a href="<?php echo $sub_menu->url; ?>"><?php echo $sub_menu->title." ".str_replace($site_url,$bucket_url,$sub_menu->description); ?></a>
										<?php if (isset($sub_menus[$sub_db_id]) && !empty($sub_menus[$sub_db_id])) { ?>
											...
											<?php foreach($sub_menus[$sub_db_id] as $subsub_db_id => $subsub_menu) { ?>
												<li>
													<a href="<?php echo $subsub_menu->url; ?>"><?php echo $subsub_menu->title." ". str_replace($site_url,$bucket_url,$sub_menu->description) ?></a>
												</li>
											<?php } ?>
											</ul>
										<?php } ?>
