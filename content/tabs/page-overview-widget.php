...
// START
// Loop "get_overview_content_repeater" from $sub_page
// Get the "Get Overview Content" sub field from Post Object
// Show the "Get Overview Content"
?>

	<?php
	$has_schedule = get_sub_field('has_schedule', $sub_page->ID);
	
	if($has_schedule)
	{		
		if(have_rows('get_overview_content_repeater', $sub_page->ID)) {	
		?>
		<section class="col-full wrapper clearfix">
			<ul class="grid-headline g-two clearfix">
			<?php
			while(have_rows('get_overview_content_repeater', $sub_page->ID)) {			
				the_row();
					
				$overview_content 	= 	get_sub_field('overview_content', $sub_page->ID);
