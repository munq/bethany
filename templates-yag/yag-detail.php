...
<section id="content" class="clearfix">
	...
	<section class="col-full wrapper clearfix">
		<?php		
		if(get_field('testimonies')) {
			
			$testimonies = get_field('testimonies');
			
			if(get_field('notes', $testimonies->ID)) {
				$notes = get_field('notes', $testimonies->ID);
				$notes_url  = ($notes) ? wp_get_attachment_url( $notes['id'] ) : '';
			} else {
				$notes_url = get_field('notes_url', $testimonies->ID);
			}
			...
			echo '<h2>' . get_the_title($testimonies->ID). '</h2>';
			if ( current_user_can('edit_post', $testimonies->ID) ) {
				edit_post_link('Edit this page', '<p>', '</p>', $testimonies->ID);
			}
			echo apply_filters('the_content', $testimonies->post_content);
		}
		else {
			...
			if(is_array($notes)) {
				...
				$notes_url	= ($notes) ? wp_get_attachment_url( $notes ) : '';
			}
	...
	<section class="col-full wrapper clearfix">
		...
		<p>&nbsp;</p>
		<p><a class="link-txt" href="<?php echo get_permalink($post->post_parent); ?>"><i class="ion-navicon-arrow"></i> Back to <?php echo get_the_title($post->post_parent); ?></a></p>
		<div class="tags-wrapper clearfix"></div>
