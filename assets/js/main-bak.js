...
$(document).ready(function(){
			...
			if (window.location.hash){
				...
				setTimeout(function(){
					if ($('html').hasClass('resp-mobile')){
						...
						$('#content').stickem({item: '.sticky-side-nav', container: '.r-tabs-panel .col-full', stickClass: 'stuck', endStickClass: 'stuck-end', offset: -200});
					}
				}, 250);
				if (!$('html').hasClass('resp-mobile')){
				}
		...
		$('.r-tabs-accordion-title > a[href^="#"]').click(function(e){
			...
			if($(current_tab).find('.sticky-side-nav').length > 0) {
				if($(current_tab).find('.sticky-side-wrapper').length < 1) {
					$(current_tab).find('.sticky-side-nav').waypoint('sticky', {offset: 0, wrapper: '<div class="sticky-side-wrapper"/>'});
				}
	...
	setTimeout(function(){
		$('#content').stickem({item: '.sticky-box', container: '.col-full', stickClass: 'stuck', endStickClass: 'stuck-end', offset: -200});
	}, 250);
	// Match the height
...
$(document).ready(function(){
	...
	my_jPlayer_2.jPlayer({
		...
		pause: function(event){
			my_playBtn_2.html('<i class="ion-ios-play" />');
			my_playState_2.text(opt_text_original_2);
		},
		ended: function(event){
			my_playBtn_2.html('<i class="ion-ios-play" />');
