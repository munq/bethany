...
/**
 * Template Name: Parenting Detail
 *
 * @package Bethany
 * @subpackage bethany
 * @since bethany 1.0
...
<section id="content" class="clearfix">
	...
	<section class="col-full wrapper clearfix">
		...
		<div id="bible-study-resources">
			...
			<?php $messages	= get_field('message', get_the_ID()); ?>
			
			<h2 class="h2-lead">Parenting by the Book</h2>			
			<?php
				...
				<form>
					<fieldset class="bsr-filter-top filter-top clearfix daily-devotion-top">
						...
						<div class="field-wrp">
							...
							?>
							<span><strong><?php echo $page_title[0] ;?></strong></span> &nbsp;
							<?php
							...
							?>	
								...
								while($query_chap->have_posts()) {			
									...
									?>
									<option value="<?php echo $post_chap->ID; //get_permalink($post_chap->ID); ?>" <?php echo ($post_chap->ID == $parent_page_id ) ? 'selected' : ''; ?>><?php echo ($chapter_title_no[1]) ? $chapter_title_no[1] : $chapter_title_no[0]; ?></option>
								<?php
									$parent_page_count++;
								} wp_reset_postdata();
								?>								
									...
									?>
										<span class="chapter-day-dropdown chapter-<?php echo $chapter_id ?>-day-dropdown hidden">
											...
											while($day_row->have_posts()) {
												...
												?>	
												<option value="<?php echo get_permalink($post_day->ID); ?>" <?php echo ($current_post_id == $post_day->ID) ? 'Selected' : ''; ?>><?php echo $title[1]; ?></option>
											<?php
											}
											?>
											</select>
							...
							<div class="devotions-select-day">
								...
								<span class="devotions-day"><strong>Study <?php echo $day_title[1]; ?></strong></span>
			...
			<section class="col-full wrapper clearfix">
				<?php
				$page_topics = get_terms('topic', get_the_ID());				
