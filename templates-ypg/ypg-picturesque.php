		...
		<section class="col-full clearfix">
			...
			if($query->have_posts()) {
				while($query->have_posts()) {
					...
					$article_title 		= get_the_title($pic_sub->ID);
					$big_picture 		= get_field('big_picture', $pic_sub->ID);	
					$author 			= get_field('author', $pic_sub->ID);
					$quote				= get_field('quote', $pic_sub->ID);
					
					if(get_field('overview_image', $pic_sub->ID)) {
						
						$overview_image 	= get_field('overview_image', $pic_sub->ID);
						$overview_image_url = ($overview_image) ? $overview_image['url'] : '' ;
					
					} else {
						$overview_image_url = get_field('overview_image_url', $pic_sub->ID);
					}

					
					?>
					<li class="box-img g-<?php echo ($big_picture) ? 'two' : 'four'?>">
						<a href="<?php echo get_permalink($pic_sub->ID);?>">
							<figure>
								<?php
