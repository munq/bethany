...
foreach ($messages as $message_id) {
    ...
    //var_dump($day_of_week_weekly);
    if ($day_of_week_weekly == 0) {
        $week_start = date("Y-m-d", strtotime("-1 day"));
        $week_end = date("Y-m-d", strtotime($week_start."+1 week"));
    } else if ($day_of_week_weekly == 6) {
        $week_start = date("Y-m-d", strtotime('now'));
    ...
    if ($query->have_posts()) {
        ...
        while ($query->have_posts()) {
            ...
            $message_args = array(
                ...
                'date_query' => array(
                    array(
                        'after' => date('Y-m-d', strtotime($week_start)),
                        'before' => date('Y-m-d', strtotime($week_end)),
                        'inclusive' => true,
                    ),
                ),
                'posts_per_page' => 1,
