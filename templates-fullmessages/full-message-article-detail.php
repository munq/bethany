...
/**
 * Template Name: Full Message Detail 
 *
...
<?php
	...
	$video_url			= get_field('full_video_url', $messages->ID);
...
<section id="content" class="clearfix">
	...
	<section class="col-full wrapper clearfix">
		<h2 class="h2-lead"><?php echo get_the_title(get_post($post->post_parent)->post_parent);?></h2>
		
		<h2><span class="numeric"><?php echo date('d',strtotime($article_date)).'  ' ;?></span><?php echo date('F',strtotime($article_date)).'  ';?><span class="numeric"><?php echo date('Y',strtotime($article_date));?></span></h2>
		...
		<section class="col-full clearfix">
			...
			<div class="download-resource clearfix">
				<div class="resource">
					<h4><strong><?php echo $article_title;?></strong><br/>
					<?php echo ($text_refrence) ? 'Text: ' .$text_refrence : '';?></h4>
					<dl class="clearfix">
					<?php 							
...
<?php get_footer();
	...
	$redirect_to = get_permalink(get_page_by_path('bethany-login')->ID);	
