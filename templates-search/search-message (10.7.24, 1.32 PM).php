...
<section id="content" class="clearfix">	
	...
	<section class="col-full wrapper clearfix">
		...
		<div id="bible-study-resources">
			...
			if(!empty($_GET['msg_year'])) {
				$msg_year_d	= $_GET['msg_year'];
			}
			
			if(!empty($_GET['msg_month'])){
				$msg_month_d = strftime("%b", mktime(0,0,0, $_GET['msg_month']));				
			}
			?>
					...
					<fieldset class="bsr-filter-top filter-top clearfix">
						<div class="field-wrp field-search">
							...
							if($msg_month_d !='' || $msg_year_d !='') { 
								echo ' Date: ' . $msg_month_d . ' ' . $msg_year_d ;
							}
							?>
							</strong></span>
						</div>
			...
			<section class="col-full wrapper clearfix">
				...
				if(isset($_GET) && !empty($_GET)) {
					...
					if(!empty($_GET['msg_search']) || !empty($_GET['msg_month']) || !empty($_GET['msg_year'])  ) {
						...
						$yearmonth = $_GET['msg_year'] . $_GET['msg_month'];
						
						if(!empty($yearmonth ) && !empty($search) ) {
						...
						if($query->have_posts()) {
							...
							while($query->have_posts()) {				
								...
								<div class="clearfix">
									...
									<p><?php echo ($speaker) ? 'by ' . $speaker : ''; ?> <?php echo ($series) ? ' | <a href="' . get_permalink($series) .'">' . get_the_title($series) . '</a>' : '';?></p>
